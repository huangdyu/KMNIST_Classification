...
def load_KMNIST_train(args):
    ...
    using image augmentation, note for this dataset, it's not suggested to do horizontal flip or vertical flip
    since we may get a different word and it has no correlation with the original word
    but we can rotate it at a small degree
    RandomApply is used since we don't wish to do transforms all of the train data
...
def load_KMNIST_test(args):
    ...
    if args.aug:
        trans_test.insert(0, transforms.GaussianBlur(kernel_size = 7, sigma = (1, 2)))
...
def class_acc(cm):
    ...
    for i in range(cm.shape[0]):
        ...
        for j in range(cm.shape[0]):
            cm_acc[i,j] = cm[i,j]/cm[i].sum()     ##using probability
...
def make_cm_plot(args, cm):
    ...
    plt.colorbar()
    if not args.aug:
        plt.savefig(f'{args.path}/{args.net}/{args.name1}')
    else:
        plt.savefig(f'{args.path}/{args.net}/{args.name2}')
    print(table)